...
#define STASSID "Little Italy's Big Ten Team"
#define STAPSK  "goB1Gcats"

#define OFFSET 50
#define R1 10000 //top of voltage divider
...
const unsigned long intervalTemp = 15000;   // Do a temperature measurement every 15 seconds
...
void loop() {
  ...
  if (timeUNIX != 0) {
    if (currentMillis - prevTemp > intervalTemp) {  // Every minute, request the temperature
      ...
      Serial.printf("Appending ADC value to file: %lu,", actualTime);
      Serial.println(temp);
...
float getTemp() { // get current temperature reading in centigrade
  int vout;
  float resistance;
  float rnorm;
  float temp;
  ...
  int read;
  read = analogRead(A0);
  vout = (read-OFFSET)*3.2227; //offset ADC reading and convert to mV 
  resistance = (R1*vout)/(VIN-vout);
  rnorm = resistance/R1;
  logrnorm = log(rnorm);
  temp = (1.0 / (A1 + B1*logrnorm + C1*logrnorm*logrnorm + D1*logrnorm*logrnorm*logrnorm))-273.15;
  return temp;
}

/*__________________________________________________________SETUP_FUNCTIONS__________________________________________________________*/

void startWiFi() { // Try to connect to an access point, then wait for a connection
